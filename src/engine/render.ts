 ...
 * (and there is no recursive expansion).
 */
export function renderStrict(text: string, vars: RenderVars, label: string): string {
  const unbound = placeholderNames(text).filter((name) => !(name in vars));
  if (unbound.length > 0) {
    throw new Error(
      `unbound template placeholder(s) in ${label}: ${unbound.map((name) => `{{${name}}}`).join(", ")} — bind them in render.vars`,
    );
  }
  return text.replace(PLACEHOLDER, (match, name: string) => vars[name] ?? match);
}
